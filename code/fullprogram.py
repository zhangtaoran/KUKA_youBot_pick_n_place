...
'''
    This program generates a csv file that can be played in vrep to show a robot doing a 
    pick and place task.
...
def TrajectoryGenerator(Tseinit,Tscinit,Tscgoal,Tcestand,Tcegrasp,k):
    ...
    a = mr.ScrewTrajectory(Tseinit,Tsestand,2,200,method)
    ...
    a = mr.ScrewTrajectory(Tsestand,Tsegrasp,0.5,50,method)
    for i in range(len(a)):
        refee = np.vstack([refee,np.array([a[i][0,0],a[i][0,1],a[i][0,2],a[i][1,0],a[i][1,1],\
            a[i][1,2],a[i][2,0],a[i][2,1],a[i][2,2],a[i][0,3],a[i][1,3],a[i][2,3],0])])

    # 1st grasp
    a = mr.ScrewTrajectory(Tsegrasp,Tsegrasp,0.7,70,method)
    for i in range(len(a)):
        refee = np.vstack([refee,np.array([a[i][0,0],a[i][0,1],a[i][0,2],a[i][1,0],a[i][1,1],\
            a[i][1,2],a[i][2,0],a[i][2,1],a[i][2,2],a[i][0,3],a[i][1,3],a[i][2,3],1])])

    # back to 1st standoff
    a = mr.ScrewTrajectory(Tsegrasp,Tsestand,0.5,50,method)
    for i in range(len(a)):
        refee = np.vstack([refee,np.array([a[i][0,0],a[i][0,1],a[i][0,2],a[i][1,0],a[i][1,1],\
            a[i][1,2],a[i][2,0],a[i][2,1],a[i][2,2],a[i][0,3],a[i][1,3],a[i][2,3],1])])

    # to final standoff
    a = mr.ScrewTrajectory(Tsestand,Tsestandg,2,200,method)
    for i in range(len(a)):
        refee = np.vstack([refee,np.array([a[i][0,0],a[i][0,1],a[i][0,2],a[i][1,0],a[i][1,1],\
            a[i][1,2],a[i][2,0],a[i][2,1],a[i][2,2],a[i][0,3],a[i][1,3],a[i][2,3],1])])

    # to final release
    a = mr.ScrewTrajectory(Tsestandg,Tsegraspg,0.5,50,method)
    for i in range(len(a)):
        refee = np.vstack([refee,np.array([a[i][0,0],a[i][0,1],a[i][0,2],a[i][1,0],a[i][1,1],\
            a[i][1,2],a[i][2,0],a[i][2,1],a[i][2,2],a[i][0,3],a[i][1,3],a[i][2,3],1])])

    # release
    a = mr.ScrewTrajectory(Tsegraspg,Tsegraspg,0.7,70,method)
    for i in range(len(a)):
        refee = np.vstack([refee,np.array([a[i][0,0],a[i][0,1],a[i][0,2],a[i][1,0],a[i][1,1],\
            a[i][1,2],a[i][2,0],a[i][2,1],a[i][2,2],a[i][0,3],a[i][1,3],a[i][2,3],0])])

    # back to final standoff
    a = mr.ScrewTrajectory(Tsegraspg,Tsestandg,0.5,50,method)
    for i in range(len(a)):
        refee = np.vstack([refee,np.array([a[i][0,0],a[i][0,1],a[i][0,2],a[i][1,0],a[i][1,1],\
            a[i][1,2],a[i][2,0],a[i][2,1],a[i][2,2],a[i][0,3],a[i][1,3],a[i][2,3],0])])
...
#controller
def feedbackcontrol(X, Xd, Xdnext, Kp, Ki, tstep, thetalist):
    #global variable for I term in pi controller
    global Xerr_integrate
    ...
    V = np.dot(mr.Adjoint(np.dot(mr.TransInv(X), Xd)), Vd) + Kp * mr.se3ToVec(mr.MatrixLog6(np.dot(mr.TransInv(X), Xd))) + Ki * Xerr_integrate
...
def runfullprogram(Tscinit,Tscgoal,config,Tseinit,Kp,Ki):
    ...
    Xerrmatrix = np.empty(shape=[6,740])
    ...
    for i in range(739):
        ...
        [v, Xerrmatrix[:,i], Xerr_integrate] = feedbackcontrol(X, Xd, Xdnext, Kp, Ki, tstep, thetalist)
    ...
    #plot Xerr
    xcord = np.linspace(0,7.38,739)
    f = plt.figure()
    plt.plot(xcord,Xerrmatrix[0,0:739],label='err_wx')
    plt.plot(xcord,Xerrmatrix[1,0:739],label='err_wy')
    plt.plot(xcord,Xerrmatrix[2,0:739],label='err_wz')
    plt.plot(xcord,Xerrmatrix[3,0:739],label='err_vx')
    plt.plot(xcord,Xerrmatrix[4,0:739],label='err_vy')
    plt.plot(xcord,Xerrmatrix[5,0:739],label='err_vz')
    plt.legend()
